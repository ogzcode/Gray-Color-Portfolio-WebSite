...
export const DroppableItem = ({ data }) => {
    ...
    return (
        <div ref={setNodeRef} className="w-[400px] h-full border border-slate-200 rounded-lg relative">
            <DroppableHeader droppableHeader={data.content} totalItem={data.items.length} />
            {
                data.items.length > 0 ? (
                    <div className="flex flex-col justify-between items-baseline gap-6 p-6">
                        {
